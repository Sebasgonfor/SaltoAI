...
| **Dashboards por rol** | Cada usuario tiene su "casa": el joven en [`/dashboard`](app/dashboard/page.tsx) (perfil + tareas activas + insight de IA), el founder en [`/empresa`](app/empresa/page.tsx) (KPIs + sus necesidades publicadas + micro-tareas en curso). Antes el founder no tenía hub: publicaba, caía en `/matches/{needId}` y no había forma de volver a su lista. El dashboard founder consume `/api/necesidad/mias` (filtrado por `ownerUid`) + `/api/microtask/list?companyId=`. `/dashboard` ahora chequea rol y redirige a `/empresa` si el usuario es founder (cerraba un agujero por el que un founder veía UI de joven). | [`app/empresa/page.tsx`](app/empresa/page.tsx) · [`app/api/necesidad/mias/route.ts`](app/api/necesidad/mias/route.ts). |
| **Perfil de Evidencia público + RoleGate per-page** | Antes [`app/joven/layout.tsx`](app/joven/layout.tsx) wrappeaba TODO `/joven/*` con `RoleGate role="joven"`. Resultado: un founder logueado como `empresa` que clickeaba una card de candidato en su dashboard chocaba contra un muro "Tu cuenta está registrada como empresa". Rompía el flujo central del producto. Fix: layout sin gate; cada page decide. `/joven/perfil/[id]` queda **público** (el perfil es contenido de matching que las empresas necesitan ver); `/joven/chat`, `/joven/tareas/*`, `/joven/conectar` envuelven con `<RoleGate role="joven">` por separado. El perfil incluye un **banner contextual** cuando `account.role === 'empresa'` ("Estás viendo el perfil de un candidato · Mis matches"). | [`app/joven/layout.tsx`](app/joven/layout.tsx) · [`app/joven/perfil/[id]/page.tsx`](app/joven/perfil/[id]/page.tsx). |
...
| **Errores de extracción** | El joven revisa su Perfil de Evidencia antes de publicarlo (human-in-the-loop). Si la extracción devuelve 0 skills + 0 evidencia, respondemos 422 "no pudimos anclar evidencia concreta — profundiza con un ejemplo". | [`app/api/perfil/route.ts`](app/api/perfil/route.ts) |
...
| `/api/necesidad/mias` | GET | Lista necesidades publicadas por el founder (filtra por `ownerUid`). Alimenta el dashboard `/empresa`. | [`app/api/necesidad/mias/route.ts`](app/api/necesidad/mias/route.ts) |

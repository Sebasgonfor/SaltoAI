...
import { getUserAccount, setUserRole, markInterviewCompleted, type UserAccount, type UserRole } from './accounts';
  ...
  useEffect(() => {
    ...
    const unsub = onAuthStateChanged(auth, async (u) => {
      ...
      try {
        ...
        if (existing && !existing.interviewCompleted) {
          try {
            const res = await fetch(`/api/perfil?id=${u.uid}`);
            if (res.ok) {
              existing = { ...existing, interviewCompleted: true };
              markInterviewCompleted(u.uid).catch(() => {});
            }
          } catch {}
        }
        setAccount(existing);
